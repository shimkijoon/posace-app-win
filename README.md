...
### 앱 업데이트 프로세스 고도화 (v1.0.18)
- **사일런트 업데이트 (Silent Mode)**: 업데이트 설치 시 인스톨러 창이 노출되지 않는 `/VERYSILENT` 모드 적용.
- **설치 후 자동 재시작**: 설치 완료 후 앱이 자동으로 다시 실행되는 워크플로우 구현 (`WizardSilent` 플래그).
- **수동 업데이트 UI 개선**:
  - 최신 버전일 경우 업데이트 버튼 비활성화.
  - `Ctrl + 클릭`으로 버전 정보 강제 업데이트 진입 기능 추가.
- **버전 릴리즈**: v1.0.18 빌드 및 GitHub 릴리즈 배포 완료.
- **상세 내용**: [앱 업데이트 최적화 요약](./APP_UPDATE_OPTIMIZATION_SUMMARY_2026-02-03.md) 참고.
